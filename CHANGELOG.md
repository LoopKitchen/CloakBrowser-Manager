...
### Added
- **Smoother live viewer on slow connections.** The viewer now receives continuous updates: the screen streams as it changes instead of one frame per round trip, which lifts a 230 ms connection from about 4 to about 50 frames per second and keeps typing and clicks as responsive on a busy screen as on a still one. KasmVNC only allows this for clients that also negotiate fences, so the proxy now forwards both instead of stripping them.
...
### Fixed
- **Live viewer disconnected when something was copied inside the browser.** KasmVNC can place its clipboard announcement in the same WebSocket frame as screen data; the proxy only looked for it at the start of a frame, so it reached the viewer untranslated and the viewer dropped with "Unexpected server message (type 180)". The proxy now walks the server stream message by message and translates the announcement wherever it appears, falling back to the previous behaviour if it ever meets a message it does not recognise. Clipboards that carry more than one format (text and HTML from a rich-text copy) are now read correctly regardless of order.
